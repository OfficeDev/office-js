...
/* office-js-api version: 20191014.8 */
/*
...
function loadLocalizedScript(initializeAppCallback) {
  ...
  } // Load outlook string file as script in the head
  ...
  if (ExtensibilityStrings_url) {
    ...
    scriptElement.onreadystatechange = scriptElementCallback; // allow 2s, inline with office.js
...
var callOutlookNativeDispatcher_convertToOutlookNativeParameters = function convertToOutlookNativeParameters(dispid, data) {
  ...
  switch (dispid) {
    ...
    /*
      ...
      case OutlookDispId.getAllInternetHeaders:
...
function getAllInternetHeaders() {
  ...
  standardInvokeHostMethod(168
  /* getAllInternetHeaders */
  , commonParameters.asyncContext, commonParameters.callback, undefined, undefined);
}
...
function CustomJsonAttachmentsResponse(arrayOfAttachmentJsonData) {
  var customJsonResponse = []; // We don't allow add-ins with restricted permissions to have access to attachments
...
// Time out to use for addItemAttachmentAsync time out.
...
function () {
  function OutlookAppOm(appContext, targetWindow, appReadyCallback) {
    ...
    }; // initialze is separated from onInitialDataResponse, because it is called from the ItemSelectionChanged event
...
/* harmony default export */ var src = __webpack_exports__["default"] = (OutlookAppOm_OutlookAppOm); // Our old files had a $h global namespace. Which really shouldn't be called. The Action Items
// default add-in calls into these two, so we define them here to avoid breaking that add-in.
// This should be removed with the next Outlook.js version. (in 16.03 Win32, 16.02 Web)
