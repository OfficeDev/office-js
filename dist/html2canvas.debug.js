...
var Html2canvas;
(function (Html2canvas) {
	var Renderer = Html2canvas.Renderer;
	var LinearGradientContainer = Html2canvas.LinearGradientContainer;
	var log = Html2canvas.Log;
	Html2canvas.CanvasRenderer = function (width, height) {
	...
	Html2canvas.CanvasRenderer.prototype = Object.create(Renderer.prototype);
	Html2canvas.CanvasRenderer.prototype.setFillStyle = function (fillStyle) {
		this.ctx.fillStyle = typeof (fillStyle) === "object" && !!fillStyle.isColor ? fillStyle.toString() : fillStyle;
		return this.ctx;
	};
	Html2canvas.CanvasRenderer.prototype.rectangle = function (left, top, width, height, color) {
		this.setFillStyle(color).fillRect(left, top, width, height);
	};
	Html2canvas.CanvasRenderer.prototype.circle = function (left, top, size, color) {
	...
	Html2canvas.CanvasRenderer.prototype.circleStroke = function (left, top, size, color, stroke, strokeColor) {
		this.circle(left, top, size, color);
		this.ctx.strokeStyle = strokeColor.toString();
		this.ctx.stroke();
	};
	Html2canvas.CanvasRenderer.prototype.drawShape = function (shape, color) {
		this.shape(shape);
		this.setFillStyle(color).fill();
	};
	Html2canvas.CanvasRenderer.prototype.taints = function (imageContainer) {
	...
	Html2canvas.CanvasRenderer.prototype.drawImage = function (imageContainer, sx, sy, sw, sh, dx, dy, dw, dh) {
		if (!this.taints(imageContainer) || this.options.allowTaint) {
			this.ctx.drawImage(imageContainer.image, sx, sy, sw, sh, dx, dy, dw, dh);
		}
	};
	Html2canvas.CanvasRenderer.prototype.clip = function (shapes, callback, context) {
		this.ctx.save();
	...
	Html2canvas.CanvasRenderer.prototype.shape = function (shape) {
		this.ctx.beginPath();
	...
	Html2canvas.CanvasRenderer.prototype.font = function (color, style, variant, weight, size, family) {
		this.setFillStyle(color).font = [style, variant, weight, size, family].join(" ").split(",")[0];
	};
	Html2canvas.CanvasRenderer.prototype.fontShadow = function (color, offsetX, offsetY, blur) {
		this.setVariable("shadowColor", color.toString())
            .setVariable("shadowOffsetY", offsetX)
            .setVariable("shadowOffsetX", offsetY)
            .setVariable("shadowBlur", blur);
	};
	Html2canvas.CanvasRenderer.prototype.clearShadow = function () {
		this.setVariable("shadowColor", "rgba(0,0,0,0)");
	};
	Html2canvas.CanvasRenderer.prototype.setOpacity = function (opacity) {
		this.ctx.globalAlpha = opacity;
	};
	Html2canvas.CanvasRenderer.prototype.setTransform = function (transform) {
		this.ctx.translate(transform.origin[0], transform.origin[1]);
		this.ctx.transform.apply(this.ctx, transform.matrix);
		this.ctx.translate(-transform.origin[0], -transform.origin[1]);
	};
	Html2canvas.CanvasRenderer.prototype.setVariable = function (property, value) {
		if (this.variables[property] !== value) {
			this.variables[property] = this.ctx[property] = value;
		}
		return this;
	};
	Html2canvas.CanvasRenderer.prototype.text = function (text, left, bottom) {
		this.ctx.fillText(text, left, bottom);
	};
	Html2canvas.CanvasRenderer.prototype.backgroundRepeatShape = function (imageContainer, backgroundPosition, size, bounds, left, top, width, height, borderData) {
		var shape = [
	...
	Html2canvas.CanvasRenderer.prototype.renderBackgroundRepeat = function (imageContainer, backgroundPosition, size, bounds, borderLeft, borderTop) {
	...
	Html2canvas.CanvasRenderer.prototype.renderBackgroundGradient = function (gradientImage, bounds) {
	...
	Html2canvas.CanvasRenderer.prototype.resizeImage = function (imageContainer, size) {
...
(function (Html2canvas) {
	var NodeContainer = Html2canvas.NodeContainer;
	Html2canvas.StackingContext = function (hasOwnStacking, opacity, element, parent) {
	...
	Html2canvas.StackingContext.prototype = Object.create(NodeContainer.prototype);
	Html2canvas.StackingContext.prototype.getParentStack = function (context) {
...
(function (Html2canvas) {
	var XHR = Html2canvas.XHR;
	var decode64 = Html2canvas.Utils.decode64;
	Html2canvas.SVGContainer = function (src) {
		this.src = src;
		this.image = null;
		var self = this;
		this.promise = this.hasFabric().then(function () {
			return (self.isInline(src) ? Html2canvas.Promise.resolve(self.inlineFormatting(src)) : XHR(src));
		}).then(function (svg) {
			return new Html2canvas.Promise(function (resolve) {
	...
	Html2canvas.SVGContainer.prototype.hasFabric = function () {
		return !window.html2canvas.svg || !window.html2canvas.svg.fabric ? Html2canvas.Promise.reject(new Error("html2canvas.svg.js is not loaded, cannot render svg")) : Html2canvas.Promise.resolve();
	};
	Html2canvas.SVGContainer.prototype.inlineFormatting = function (src) {
		return (/^data:image\/svg\+xml;base64,/.test(src)) ? this.decode64(this.removeContentType(src)) : this.removeContentType(src);
	};
	Html2canvas.SVGContainer.prototype.removeContentType = function (src) {
		return src.replace(/^data:image\/svg\+xml(;base64)?,/, '');
	};
	Html2canvas.SVGContainer.prototype.isInline = function (src) {
		return (/^data:image\/svg\+xml/i.test(src));
	};
	Html2canvas.SVGContainer.prototype.createCanvas = function (resolve) {
		var self = this;
	...
	Html2canvas.SVGContainer.prototype.decode64 = function (str) {
		return (typeof (window.atob) === "function") ? window.atob(str) : decode64(str);
	};
})(Html2canvas || (Html2canvas = {}));
var Html2canvas;
(function (Html2canvas) {
	Html2canvas.SVGNodeContainer = function (node, _native) {
		this.src = node;
		this.image = null;
		var self = this;
		this.promise = _native ? new Html2canvas.Promise(function (resolve, reject) {
	...
	Html2canvas.SVGNodeContainer.prototype = Object.create(Html2canvas.SVGContainer.prototype);
})(Html2canvas || (Html2canvas = {}));
var Html2canvas;
(function (Html2canvas) {
	var NodeContainer = Html2canvas.NodeContainer;
	Html2canvas.TextContainer = function (node, parent) {
		NodeContainer.call(this, node, parent);
	};
	Html2canvas.TextContainer.prototype = Object.create(NodeContainer.prototype);
	Html2canvas.TextContainer.prototype.applyTextTransform = function () {
		this.node.data = this.transform(this.parent.css("textTransform"));
	};
	Html2canvas.TextContainer.prototype.transform = function (transform) {
...
(function (Html2canvas) {
	var GradientContainer = Html2canvas.GradientContainer;
	Html2canvas.WebkitGradientContainer = function (imageData) {
		GradientContainer.apply(this, arguments);
		this.type = imageData.args[0] === "linear" ? GradientContainer.TYPES.LINEAR : GradientContainer.TYPES.RADIAL;
	};
	Html2canvas.WebkitGradientContainer.prototype = Object.create(GradientContainer.prototype);
})(Html2canvas || (Html2canvas = {}));
var Html2canvas;
(function (Html2canvas) {
	var log = Html2canvas.Log;
	var punycode = PunyCode.PunyCode;
	var NodeContainer = Html2canvas.NodeContainer;
	var TextContainer = Html2canvas.TextContainer;
	var PseudoElementContainer = Html2canvas.PseudoElementContainer;
	var FontMetrics = Html2canvas.FontMetrics;
	var Color = Html2canvas.Color;
	var StackingContext = Html2canvas.StackingContext;
	var utils = Html2canvas.Utils;
	var bind = utils.bind;
	var getBounds = utils.getBounds;
	var parseBackgrounds = utils.parseBackgrounds;
	var offsetBounds = utils.offsetBounds;
	Html2canvas.NodeParser = function (element, renderer, support, imageLoader, options) {
		...
		this.ready = this.images.ready.then(bind(function () {
			...
			return new Html2canvas.Promise(bind(function (resolve) {
				if (!options.async) {
	...
	Html2canvas.NodeParser.prototype.calculateOverflowClips = function () {
	...
	Html2canvas.NodeParser.prototype.asyncRenderer = function (queue, resolve, asyncTimer) {
	...
	Html2canvas.NodeParser.prototype.createPseudoHideStyles = function (document) {
		this.createStyles(document, '.' + PseudoElementContainer.prototype.PSEUDO_HIDE_ELEMENT_CLASS_BEFORE + ':before { content: "" !important; display: none !important; }' +
            '.' + PseudoElementContainer.prototype.PSEUDO_HIDE_ELEMENT_CLASS_AFTER + ':after { content: "" !important; display: none !important; }');
	};
	Html2canvas.NodeParser.prototype.disableAnimations = function (document) {
		this.createStyles(document, '* { -webkit-animation: none !important; -moz-animation: none !important; -o-animation: none !important; animation: none !important; ' +
            '-webkit-transition: none !important; -moz-transition: none !important; -o-transition: none !important; transition: none !important;}');
	};
	Html2canvas.NodeParser.prototype.createStyles = function (document, styles) {
		var hidePseudoElements = document.createElement('style');
		hidePseudoElements.innerHTML = styles;
		document.body.appendChild(hidePseudoElements);
	};
	Html2canvas.NodeParser.prototype.getPseudoElements = function (container) {
		var nodes = [[container]];
	...
	Html2canvas.NodeParser.prototype.getPseudoElement = function (container, type) {
	...
	Html2canvas.NodeParser.prototype.getChildren = function (parentContainer) {
		return flatten([].filter.call(parentContainer.node.childNodes, renderableNode).map(function (node) {
			var container = [node.nodeType === Node.TEXT_NODE ? new TextContainer(node, parentContainer) : new NodeContainer(node, parentContainer)].filter(nonIgnoredElement);
			return node.nodeType === Node.ELEMENT_NODE && container.length && node.tagName !== "TEXTAREA" ? (container[0].isElementVisible() ? container.concat(this.getChildren(container[0])) : []) : container;
		}, this));
	};
	Html2canvas.NodeParser.prototype.newStackingContext = function (container, hasOwnStacking) {
	...
	Html2canvas.NodeParser.prototype.createStackingContexts = function () {
		this.nodes.forEach(function (container) {
	...
	Html2canvas.NodeParser.prototype.isBodyWithTransparentRoot = function (container) {
		return container.node.nodeName === "BODY" && container.parent.color('backgroundColor').isTransparent();
	};
	Html2canvas.NodeParser.prototype.isRootElement = function (container) {
		return container.parent === null;
	};
	Html2canvas.NodeParser.prototype.sortStackingContexts = function (stack) {
		stack.contexts.sort(zIndexSort(stack.contexts.slice(0)));
		stack.contexts.forEach(this.sortStackingContexts, this);
	};
	Html2canvas.NodeParser.prototype.parseTextBounds = function (container) {
	...
	Html2canvas.NodeParser.prototype.getWrapperBounds = function (node, transform) {
	...
	Html2canvas.NodeParser.prototype.getRangeBounds = function (node, offset, length) {
	...
	Html2canvas.NodeParser.prototype.parse = function (stack) {
	...
	Html2canvas.NodeParser.prototype.paint = function (container) {
		try {
	...
	Html2canvas.NodeParser.prototype.paintNode = function (container) {
		if (isStackingContext(container)) {
	...
	Html2canvas.NodeParser.prototype.paintElement = function (container) {
		...
		this.renderer.clip(container.backgroundClip, function () {
			switch (container.node.nodeName) {
				...
				case "INPUT":
				case "TEXTAREA":
					if (container.node.nodeName === 'INPUT' && container.node.type === 'image') {
						var imgContainer = this.images.get(container.node);
						if (imgContainer) {
							this.renderer.renderImage(container, bounds, container.borders, imgContainer);
						}
	...
	Html2canvas.NodeParser.prototype.paintCheckbox = function (container) {
	...
	Html2canvas.NodeParser.prototype.paintRadio = function (container) {
		var bounds = container.parseBounds();
	...
	Html2canvas.NodeParser.prototype.paintFormValue = function (container) {
	...
	Html2canvas.NodeParser.prototype.paintText = function (container) {
	...
	Html2canvas.NodeParser.prototype.renderTextDecoration = function (container, bounds, metrics) {
	...
	Html2canvas.NodeParser.prototype.parseBorders = function (container) {
	...
	Html2canvas.NodeParser.prototype.parseBackgroundClip = function (container, borderPoints, borders, radius, bounds) {
	...
	function calculateCurvePoints(bounds, borderRadius, borders) {
		var x = bounds.left, y = bounds.top, width = bounds.width, height = bounds.height, tlh = borderRadius[0][0] < width / 2 ? borderRadius[0][0] : width / 2, tlv = borderRadius[0][1] < height / 2 ? borderRadius[0][1] : height / 2, trh = borderRadius[1][0] < width / 2 ? borderRadius[1][0] : width / 2, trv = borderRadius[1][1] < height / 2 ? borderRadius[1][1] : height / 2, brh = borderRadius[2][0] < width / 2 ? borderRadius[2][0] : width / 2, brv = borderRadius[2][1] < height / 2 ? borderRadius[2][1] : height / 2, blh = borderRadius[3][0] < width / 2 ? borderRadius[3][0] : width / 2, blv = borderRadius[3][1] < height / 2 ? borderRadius[3][1] : height / 2;
...
(function (Html2canvas) {
	var utils = Html2canvas.Utils;
	var getBounds = utils.getBounds;
	var loadUrlDocument = Html2canvas.loadUrlDocument;
	Html2canvas.FrameContainer = function (container, sameOrigin, options) {
		this.image = null;
		this.src = container;
		var self = this;
		var bounds = getBounds(container);
		this.promise = (!sameOrigin ? this.proxyLoad(options.proxy, bounds, options) : new Html2canvas.Promise(function (resolve) {
			...
			var html2canvas = Html2canvas.html2canvasExport;
			return html2canvas(container.contentWindow.document.documentElement, { type: 'view', width: container.width, height: container.height, proxy: options.proxy, javascriptEnabled: options.javascriptEnabled, removeContainer: options.removeContainer, allowTaint: options.allowTaint, imageTimeout: options.imageTimeout / 2 });
		}).then(function (canvas) {
			return self.image = canvas;
		});
	};
	Html2canvas.FrameContainer.prototype.proxyLoad = function (proxy, bounds, options) {
...
(function (Html2canvas) {
	var log = Html2canvas.Log;
	var ImageContainer = Html2canvas.ImageContainer;
	var DummyImageContainer = Html2canvas.DummyImageContainer;
	var ProxyImageContainer = Html2canvas.ProxyImageContainer;
	var FrameContainer = Html2canvas.FrameContainer;
	var SVGContainer = Html2canvas.SVGContainer;
	var SVGNodeContainer = Html2canvas.SVGNodeContainer;
	var LinearGradientContainer = Html2canvas.LinearGradientContainer;
	var WebkitGradientContainer = Html2canvas.WebkitGradientContainer;
	var bind = Html2canvas.Utils.bind;
	Html2canvas.ImageLoader = function (options, support) {
		this.link = null;
		this.options = options;
		this.support = support;
		this.origin = this.getOrigin(window.location.href);
	};
	Html2canvas.ImageLoader.prototype.findImages = function (nodes) {
		var images = [];
	...
	Html2canvas.ImageLoader.prototype.findBackgroundImage = function (images, container) {
		container.parseBackgroundImages().filter(this.hasImageBackground).forEach(this.addImage(images, this.loadImage), this);
		return images;
	};
	Html2canvas.ImageLoader.prototype.addImage = function (images, callback) {
	...
	Html2canvas.ImageLoader.prototype.hasImageBackground = function (imageData) {
		return imageData.method !== "none";
	};
	Html2canvas.ImageLoader.prototype.loadImage = function (imageData) {
	...
	Html2canvas.ImageLoader.prototype.isSVG = function (src) {
		return src.substring(src.length - 3).toLowerCase() === "svg" || SVGContainer.prototype.isInline(src);
	};
	Html2canvas.ImageLoader.prototype.imageExists = function (images, src) {
		return images.some(function (image) {
			return image.src === src;
		});
	};
	Html2canvas.ImageLoader.prototype.isSameOrigin = function (url) {
		return (this.getOrigin(url) === this.origin);
	};
	Html2canvas.ImageLoader.prototype.getOrigin = function (url) {
		var link = this.link || (this.link = document.createElement("a"));
		link.href = url;
		link.href = link.href;
		return link.protocol + link.hostname + link.port;
	};
	Html2canvas.ImageLoader.prototype.getPromise = function (container) {
	...
	Html2canvas.ImageLoader.prototype.get = function (src) {
		var found = null;
		return this.images.some(function (img) {
			return (found = img).src === src;
		}) ? found : null;
	};
	Html2canvas.ImageLoader.prototype.fetch = function (nodes) {
		...
		this.ready = Html2canvas.Promise.all(this.images.map(this.getPromise, this));
		log("Finished searching images");
		return this;
	};
	Html2canvas.ImageLoader.prototype.timeout = function (container, timeout) {
		var timer;
		var promise = Html2canvas.Promise.race([container.promise, new Html2canvas.Promise(function (res, reject) {
...
var Html2canvas;
(function (Html2canvas) {
	var Support = Html2canvas.Support;
	var CanvasRenderer = Html2canvas.CanvasRenderer;
	var ImageLoader = Html2canvas.ImageLoader;
	var NodeParser = Html2canvas.NodeParser;
	var NodeContainer = Html2canvas.NodeContainer;
	var log = Html2canvas.Log;
	var utils = Html2canvas.Utils;
	var createWindowClone = Html2canvas.CreateWindowClone;
	...
	Html2canvas.html2canvas = function (nodeList, options) {
		...
		if (typeof (nodeList) === "string") {
			if (typeof (options.proxy) !== "string") {
				return Html2canvas.Promise.reject("Proxy must be used when rendering url");
			}
	...
	window.html2canvas = Html2canvas.html2canvas;
	Html2canvas.html2canvas.CanvasRenderer = CanvasRenderer;
	Html2canvas.html2canvas.NodeContainer = NodeContainer;
	Html2canvas.html2canvas.log = log;
	Html2canvas.html2canvas.utils = utils;
	Html2canvas.html2canvasExport = (typeof (document) === "undefined" || typeof (Object.create) !== "function" || typeof (document.createElement("canvas").getContext) !== "function") ? function () {
		return Html2canvas.Promise.reject("No canvas support");
	} : Html2canvas.html2canvas;
	function renderDocument(document, options, windowWidth, windowHeight, html2canvasIndex) {
		return createWindowClone(document, document, windowWidth, windowHeight, options, document.defaultView.pageXOffset, document.defaultView.pageYOffset).then(function (container) {
			...
			var oncloneHandler = (typeof (options.onclone) === "function") ? Html2canvas.Promise.resolve(options.onclone(clonedWindow.document)) : Html2canvas.Promise.resolve(true);
