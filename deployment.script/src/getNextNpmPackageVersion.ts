 ...
 */
export function getNextVersionTag(tag: string, currentVersionRelease?: string, currentVersionTag?: string) {
...
export function test() {
    ...
    if (!(getNextVersionRelease(undefined) === "0.0.0"
    ...
    type TestParameters = {tag: string, releaseVersion?: string, tagVersion?: string, expectedVersion: string};
    const tests: TestParameters[] = [
        {tag: "tag", releaseVersion: undefined, tagVersion:undefined, expectedVersion:"0.0.1-tag.0"}, // first time
        {tag: "tag", releaseVersion: "0.0.0", tagVersion:"0.0.1-tag.0", expectedVersion:"0.0.1-tag.1"}, // increment tag version
        {tag: "tag", releaseVersion: "0.0.0", tagVersion:"1.0.1-tag.0", expectedVersion:"1.0.1-tag.1"}, // Follow tag version
        {tag: "tag", releaseVersion: "1.0.0", tagVersion:"0.0.0-tag.0", expectedVersion:"1.0.1-tag.0"}, // Follow release version
        {tag: "tag", releaseVersion: "1.0.0", tagVersion:"1.0.0-tag.1", expectedVersion:"1.0.1-tag.0"}, // Follow release version
    ];
    ...
    tests.forEach((t: TestParameters) => {
        ...
        if (actualVersion !== t.expectedVersion) {
            failures.push(`failed: ${JSON.stringify(t)} expected: [${t.expectedVersion}] actual: [${actualVersion}]`)
        }
    });
